...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h, dff=2048, dropout=.1):
        ...
        self.Freq_embedding = nn.Linear(d_model, d_model)

        self.fc = nn.Sequential(*[nn.Linear(d_model, dff), nn.ReLU(inplace=True), nn.Dropout(p=dropout),
                                  nn.Linear(dff, d_model)])
        self.fc_freq = nn.Sequential(*[nn.Linear(d_model, dff), nn.ReLU(inplace=True), nn.Dropout(p=dropout),
                                  nn.Linear(dff, d_model)])

    def forward(self, queries, keys, values):
        b, f, c = queries.shape
        # att = self.layer_norm2(queries)
        att = self.attention(queries, keys, values)
        att = self.dropout(att)
        # att = self.layer_norm( att)
        att1 = self.fc(att[:, :f//2])
        f1 = dct.dct_2d(att[:, f//2:].permute(0, 2, 1)).permute(0, 2, 1).contiguous()

        # f1 = dct.dct(att[:, f//2:].permute(0, 2, 1))[:, :,:f//4].permute(0, 2, 1).contiguous()
        # q_f= self.Freq_embedding(queries)
        # q_f = dct.dct(q_f.permute(0, 2, 1))[:, :,:f//4].permute(0, 2, 1).contiguous()
   
        att2 = self.fc_freq(f1)
        att2 = dct.idct_2d(att2.permute(0, 2, 1)).permute(0, 2, 1).contiguous()

        att = self.dropout(torch.cat((att1, att2), dim=1))
        # print(att.shape)
...
class EncoderSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        in_encoder = x + sinusoid_encoding_table(x.shape[1], x.shape[2]).expand(x.shape).cuda(device=2)
        # in_encoder= self.layer_norm2(in_encoder)
