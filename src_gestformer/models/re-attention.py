...
class ScaledDotProductAttention(nn.Module):
    ...
    def __init__(self, d_model, d_k, d_v, h):
        ...
        self.d_v = d_v
        self.h = h
        self.reattn_weights = nn.Parameter(torch.randn(h, h))
    ...
    def forward(self, queries, keys, values):
        ...
        # print(att.shape)
        # print(self.reattn_weights.shape)

        att = torch.softmax(att, -1)
        attn = torch.einsum('b h i j, h g -> b g i j', att, self.reattn_weights)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, queries, keys, values):
        ...
        # # print(att.shape)
        # att = self.layer_norm(queries + att)
        # att = torch.fft.fft(torch.fft.fft(att, dim=-1), dim=-2)
