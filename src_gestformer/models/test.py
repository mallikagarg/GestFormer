...
class GestureTest(object):
    ...
    def __test(self):
        ...
        tot = 0
        with torch.no_grad():
            with open('csv/NVGestures/new.csv', 'a', newline='') as csvfile:
                for i, data_tuple in enumerate(tqdm(self.data_loader, desc="Test")):
                    ...
                    hist_df = pd.DataFrame(arr)
                    # print(hist_df)
                    #log_file.close()
                    hist_df.to_csv(csvfile, mode='a',header=False, index=False)

                    correct = gt.detach().squeeze(dim=1)
                    #print(gt)
                    #hist_df = pd.DataFrame(correct)
                    #hist_df.to_csv(csvfile, mode='a',header=False)
