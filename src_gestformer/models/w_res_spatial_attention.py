...
class EncoderSelfAttention(nn.Module):
    def __init__(self, d_model, d_k, d_v, n_head, dff=2048, dropout_transformer=.1, n_module=6):
        ...
        # self.Spatial_pos_embed = nn.Linear(d_model, 4*d_model)
        ...
        self.spatial_encoder = nn.ModuleList([MultiHeadAttention(40, 5, 5, n_head, dff, dropout_transformer)
                                      for _ in range(n_module)])                              

    def forward(self, x):
        # print(x.shape)   #8,40,512
        # b, f, p = x.shape  ##### b is batch size, f is number of frames, p is dmodel
        # x = rearrange(x, 'b f p  -> (b f) p', ) 
        # # print(x.shape)
        # # print(self.Spatial_pos_embed(x).shape )

        # x = self.Spatial_pos_embed(x)  
        # # print(x.shape)
        # for l in self.spatial_encoder:
        #     x = l(x, x, x)
        # # print(x.shape) 
        # x = rearrange(x, '(b f) p -> b p f ', f=f,)
        # # print(x.shape)
        ...
        x = x + sinusoid_encoding_table(x.shape[1], x.shape[2]).expand(x.shape).cuda()
        # print(x.shape)
        for l in self.spatial_encoder:
            x = l(x, x, x)
        # print(x.shape) 
        x = rearrange(x, 'b p f -> b f p ', f=f,)
        # print(x.shape)
        # x = self.weighted_mean(x)
        # x = x.view(b, f, -1)
